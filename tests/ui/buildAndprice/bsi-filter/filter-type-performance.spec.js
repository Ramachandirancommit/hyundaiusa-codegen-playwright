...
test('BSI Filter - Performance Name Validation (Data Driven)', async ({ page }) => {
  ...
  console.log('Performance filter applied 🎯');
  ...
  console.log(`Performance cards found: ${count}`);

  // =========================
  // FILTER PERFORMANCE VEHICLES FROM DATA
  // =========================
  const performanceVehicles = type.filter(v =>
    v.type.includes('performance')
  );

  console.log(`Performance vehicles in dataset: ${performanceVehicles.length}`);

  expect(count).toBe(performanceVehicles.length);

  // =========================
  // DATA DRIVEN VALIDATION
  // =========================
  for (const vehicle of performanceVehicles) {
  ...
  console.log('\n🎉 ALL PERFORMANCE VEHICLES VALIDATED SUCCESSFULLY 🎉\n');
});